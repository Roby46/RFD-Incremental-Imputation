...
# Carica il file XML
tree = ET.parse(f'XML Files/Hospital_Staff_similarity_rules.xml')
root = tree.getroot()
...
if(approach=="Baseline"):
    df=pd.read_csv(f"../Imputation_Results/Imputation_Baseline_Results/Hospital_Staff_6000/1/Baseline_Hospital_Staff_6000_{MV}_{version}.csv", sep=';')
elif(approach=="Pipeline_noRev"):
    df = pd.read_csv(f"../Imputation_Results/Imputation_noRestoring_Results/Hospital_Staff_6000/{version}/Hospital_Staff_6000_{MV}_{version}.csv", sep=';')
elif(approach=="Baseline20"):
    df = pd.read_csv(f"../Imputation_Results/Imputation_Baseline_20_Results/Hospital_Staff_6000/{version}/Baseline_Hospital_Staff_6000_{MV}_{version}.csv", sep=';')
elif(approach=="Pipeline"):
    df = pd.read_csv(f"../Imputation_Results/Imputation_Pipeline_Results/Hospital_Staff_6000/{version}/Hospital_Staff_6000_{MV}_{version}.csv", sep=';')
...
row_data=["Hospital_Staff_6000"]+[approach]+[MV]+[str(recall)]+[str(precision)]+[str(version)]+[str(exact)] + [str(similar)] + [str(wrong)]
print(row_data)
