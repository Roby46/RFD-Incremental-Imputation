...
# Carica il file XML
tree = ET.parse(f'XML Files/EV_Vehicles_similarity_rules.xml')
root = tree.getroot()
...
def checkResults(version, approach, MV):
    df = ""
    if (approach == "Baseline"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Baseline_Results/EV_Vehicles_4000/{version}/Baseline_EV_Vehicles_4000_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Pipeline_noRev"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_noRestoring_Results/EV_Vehicles_4000/{version}/EV_Vehicles_4000_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Baseline20"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Baseline_20_Results/EV_Vehicles_4000/{version}/Baseline_EV_Vehicles_4000_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Pipeline"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Pipeline_Results/EV_Vehicles_4000/{version}/EV_Vehicles_4000_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Hybrid"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Hybrid_Results/EV_Vehicles_4000/{version}/EV_Vehicles_4000_{MV}_{version}.csv",
            sep=';')
    ...
    row_data = ["EV_Vehicles_4000"] + [approach] + [MV] + [str(recall)] + [str(precision)] + [str(version)] + [
        str(exact)] + [str(similar)] + [str(wrong)]
    print(row_data)
...
for version in versions:
    for approach in approaches:
        for mv in MVs:
            checkResults(version,approach,mv)
