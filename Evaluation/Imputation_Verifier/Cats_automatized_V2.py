...
xml_file = 'XML Files/Cats_similarity_rules.xml'
# Carica il file XML
...
def process_dataset(dataset_path, results_path, header_file, dataset_name, full_dataset_path,xml_file):
    ...
    string_columns = (
            full_dataset.select_dtypes(include=['object']).columns.tolist()
...
def analyze_imputation_quality(version, approach, MV, xml_file):
    dataset_name = f"Cats_1071_{MV}_{version}"

    # Esempio di utilizzo
    missing_dataset_path = f'../../Datasets/Missing_Datasets/Cats_1071/{dataset_name}.csv'
    full_dataset_path = f'../../Datasets/Preprocessed_Datasets/Cats_1071.csv'

    results_path = ""
    if (approach == "Baseline"):
        results_path = f'../Imputation_Results/Imputation_Baseline_Results/Cats_1071/{version}/Baseline_{dataset_name}.csv'
    elif (approach == "Pipeline_noRev"):
        results_path = f'../Imputation_Results/Imputation_noRestoring_Results/Cats_1071/{version}/{dataset_name}.csv'
    elif (approach == "Baseline20"):
        results_path = f'../Imputation_Results/Imputation_Baseline_20_Results/Cats_1071/{version}/Baseline_{dataset_name}.csv'
    elif (approach == "Pipeline"):
        results_path = f'../Imputation_Results/Imputation_Pipeline_Results/Cats_1071/{version}/{dataset_name}.csv'
    elif (approach == "Hybrid"):
        results_path = f'../Imputation_Results/Imputation_Hybrid_Results/Cats_1071/{version}/{dataset_name}.csv'
...
def checkResults(version, approach, MV,xml_file):
    ...
    if (approach == "Baseline"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Baseline_Results/Cats_1071/{version}/Baseline_Cats_1071_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Pipeline_noRev"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_noRestoring_Results/Cats_1071/{version}/Cats_1071_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Baseline20"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Baseline_20_Results/Cats_1071/{version}/Baseline_Cats_1071_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Pipeline"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Pipeline_Results/Cats_1071/{version}/Cats_1071_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Hybrid"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Hybrid_Results/Cats_1071/{version}/Cats_1071_{MV}_{version}.csv",
            sep=';')
    ...
    row_data = ["Cats_1071"] + [approach] + [MV] + [str(recall)] + [str(precision)] + [str(version)] + [
...
versions=[1,2,3,4,5]
MVs =  [107, 214 ,321 ,428 ,536 ,1071,2142,3213,4284,5355]
