...
from Evaluation.Imputation_Verifier.Weather import precision

xml_file = 'XML Files/cleveland_similarity_rules.xml'
# Carica il file XML
...
def analyze_imputation_quality(version, approach, MV, xml_file):
    dataset_name = f"Cleveland_{MV}_{version}"

    # Esempio di utilizzo
    missing_dataset_path = f'../../Datasets/Missing_Datasets/Cleveland/{dataset_name}.csv'
    full_dataset_path = f'../../Datasets/Preprocessed_Datasets/Cleveland.csv'

    results_path = ""
    if (approach == "Baseline"):
        results_path = f'../Imputation_Results/Imputation_Baseline_Results/Cleveland/{version}/Baseline_{dataset_name}.csv'
    elif (approach == "Pipeline_noRev"):
        results_path = f'../Imputation_Results/Imputation_noRestoring_Results/Cleveland/{version}/{dataset_name}.csv'
    elif (approach == "Baseline20"):
        results_path = f'../Imputation_Results/Imputation_Baseline_20_Results/Cleveland/{version}/Baseline_{dataset_name}.csv'
    elif (approach == "Pipeline"):
        results_path = f'../Imputation_Results/Imputation_Pipeline_Results/Cleveland/{version}/{dataset_name}.csv'
    elif (approach == "Hybrid"):
        results_path = f'../Imputation_Results/Imputation_Hybrid_Results/Cleveland/{version}/{dataset_name}.csv'
...
def checkResults(version, approach, MV,xml_file):
    ...
    if (approach == "Baseline"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Baseline_Results/Cleveland/{version}/Baseline_Cleveland_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Pipeline_noRev"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_noRestoring_Results/Cleveland/{version}/Cleveland_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Baseline20"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Baseline_20_Results/Cleveland/{version}/Baseline_Cleveland_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Pipeline"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Pipeline_Results/Cleveland/{version}/Cleveland_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Hybrid"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Hybrid_Results/Cleveland/{version}/Cleveland_{MV}_{version}.csv",
            sep=';')
    ...
    row_data = ["Cleveland"] + [approach] + [MV] + [str(recall)] + [str(precision)] + [str(version)] + [
...
versions=[1,2,3,4,5]
MVs=[42, 83, 125, 166,208, 416, 832, 1247, 1663, 2079]
