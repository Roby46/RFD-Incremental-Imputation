...
xml_file = 'XML Files/Students_similarity_rules.xml'
# Carica il file XML
...
def analyze_imputation_quality(version, approach, MV, xml_file):
    dataset_name = f"Student_908_{MV}_{version}"

    # Esempio di utilizzo
    missing_dataset_path = f'../../Datasets/Missing_Datasets/Student_908/{dataset_name}.csv'
    full_dataset_path = f'../../Datasets/Preprocessed_Datasets/Student_908.csv'

    results_path = ""
    if (approach == "Baseline"):
        results_path = f'../Imputation_Results/Imputation_Baseline_Results/Student_908/{version}/Baseline_{dataset_name}.csv'
    elif (approach == "Pipeline_noRev"):
        results_path = f'../Imputation_Results/Imputation_noRestoring_Results/Student_908/{version}/{dataset_name}.csv'
    elif (approach == "Baseline20"):
        results_path = f'../Imputation_Results/Imputation_Baseline_20_Results/Student_908/{version}/Baseline_{dataset_name}.csv'
    elif (approach == "Pipeline"):
        results_path = f'../Imputation_Results/Imputation_Pipeline_Results/Student_908/{version}/{dataset_name}.csv'
    elif (approach == "Hybrid"):
        results_path = f'../Imputation_Results/Imputation_Hybrid_Results/Student_908/{version}/{dataset_name}.csv'
...
def checkResults(version, approach, MV,xml_file):
    ...
    if (approach == "Baseline"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Baseline_Results/Student_908/{version}/Baseline_Student_908_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Pipeline_noRev"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_noRestoring_Results/Student_908/{version}/Student_908_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Baseline20"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Baseline_20_Results/Student_908/{version}/Baseline_Student_908_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Pipeline"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Pipeline_Results/Student_908/{version}/Student_908_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Hybrid"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Hybrid_Results/Student_908/{version}/Student_908_{MV}_{version}.csv",
            sep=';')
    ...
    row_data = ["Student_908"] + [approach] + [MV] + [str(recall)] + [str(precision)] + [str(version)] + [
        str(exact)] + [str(similar)] + [str(wrong)] + [rmse]
    print(row_data)
    write_unique_row_to_csv(all_results_path, row_data)


approaches=["Pipeline","Pipeline_noRev"] #,"Baseline20","Hybrid"]
approaches=["Hybrid"]
versions=[1,2,3,4,5]
MVs =  [100 ,200 ,300 ,400 ,499 ,999,1998,2996,3995,4994]
