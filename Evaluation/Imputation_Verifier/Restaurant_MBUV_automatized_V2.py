...
def analyze_imputation_quality(version, approach, MV, xml_file):
    dataset_name = f"restaurant_MBUV_{MV}_{version}"

    # Esempio di utilizzo
    missing_dataset_path = f'../../Datasets/Missing_Datasets/restaurant_MBUV/{dataset_name}.csv'
    full_dataset_path = f'../../Datasets/Preprocessed_Datasets/restaurant_MBUV.csv'

    results_path = ""
    if (approach == "Baseline"):
        results_path = f'../Imputation_Results/Imputation_Baseline_Results/restaurant_MBUV/{version}/Baseline_{dataset_name}.csv'
    elif (approach == "Pipeline_noRev"):
        results_path = f'../Imputation_Results/Imputation_noRestoring_Results/restaurant_MBUV/{version}/{dataset_name}.csv'
    elif (approach == "Baseline20"):
        results_path = f'../Imputation_Results/Imputation_Baseline_20_Results/restaurant_MBUV/{version}/Baseline_{dataset_name}.csv'
    elif (approach == "Pipeline"):
        results_path = f'../Imputation_Results/Imputation_Pipeline_Results/restaurant_MBUV/{version}/{dataset_name}.csv'
    elif (approach == "Hybrid"):
        results_path = f'../Imputation_Results/Imputation_Hybrid_Results/restaurant_MBUV/{version}/{dataset_name}.csv'
...
def checkResults(version, approach, MV,xml_file):
    ...
    if (approach == "Baseline"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Baseline_Results/restaurant_MBUV/{version}/Baseline_restaurant_MBUV_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Pipeline_noRev"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_noRestoring_Results/restaurant_MBUV/{version}/restaurant_MBUV_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Baseline20"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Baseline_20_Results/restaurant_MBUV/{version}/Baseline_restaurant_MBUV_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Pipeline"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Pipeline_Results/restaurant_MBUV/{version}/restaurant_MBUV_{MV}_{version}.csv",
            sep=';')
    elif (approach == "Hybrid"):
        df = pd.read_csv(
            f"../Imputation_Results/Imputation_Hybrid_Results/restaurant_MBUV/{version}/restaurant_MBUV_{MV}_{version}.csv",
            sep=';')
    ...
    row_data = ["restaurant_MBUV"] + [approach] + [MV] + [str(recall)] + [str(precision)] + [str(version)] + [
